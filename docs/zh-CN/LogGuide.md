...
需要注意的是，在大部分副本里，同一个场景中会出现很多名字相同的怪物实体。 例如在T13中，区域中有大约20个至尊巴哈姆特，大部分是不可见的。 通常您可以通过它们的HP数值区分可见与否 (更多请查看 [AddCombatant](#03-addcombatant) 日志行)。 这些不可见的怪物通常是副本内造成实际伤害的“角色”，这也是为何在绝神兵中，迦楼罗和泰坦都会使用花岗岩牢狱将玩家囚禁。

### 技能ID

尽管在FF14中能力技和魔法战技是完全不相同的概念，但本文档对这些词汇将不做区分。 本文将玩家或敌人执行的所有动作均称为“技能”，每个技能拥有各不相同独立的4字节ID。

当解析插件还未更新的时候会将一些技能标为“Unknown”，此时您可以使用 xivapi.com 来查找这些特定的技能到底是什么。 例如，炽炎的技能ID是0xDF9 (十进制为3577)，您可以通过下面这个链接得知更多信息： <https://xivapi.com/action/3577?columns=ID,Name,Description,ClassJobCategory.Name>
...
这些就是本文档所谓的“游戏日志行”。 最前面是两字节的日志类型代码，其后跟着一个字符串。 由于触发器中很少使用游戏日志行 （除了 `0839` 类型的信息）， 因此并没有详细地记录所有日志类型(LogTypes)。

（欢迎补完。）

#### 不要针对游戏日志行编写触发器

建议尽量不要基于游戏日志行编写触发器，原因如下：

* 游戏日志行通常比ACT日志行晚半秒钟才显示
