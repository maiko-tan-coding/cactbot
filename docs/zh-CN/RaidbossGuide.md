...
**zoneId** 区域名缩写，用于指定触发器的适用区域。 这些区域名缩写可以在 [zone_id.ts](../resources/zone_id.ts) 文件中找到。 我们倾向于使用该属性，而非zoneRegex。 每个触发器集合都必须包含zoneId或zoneRegex之一(但二者不能并存)。
...
(理论上，以后我们可能不再需要独立的语言特定正则表达式， (理论上，以后我们可能不再需要独立的语言特定正则表达式， 而是采用 `timelineReplace` 对象自动地替换这些正则表达式。 我们还没有确定具体的实现方式，但条条大路通罗马。)
