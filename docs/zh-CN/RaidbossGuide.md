...
但是同时使用 `response` 和 `outputStrings` 的触发器则略有不同。 在触发器本身不应当设置 `outputStrings` 字段，而是在 `response` 返回的函数中设置 `output.responseOutputStrings = {};`。 其中 `{}` 是原本应当在 `outputStrings` 字段返回的对象。 这种方式看起来有些笨拙，但这样让response不仅可以直接使用，也可以使用 `outputStrings`，并保持 [resources/responses.js](../resources/responses.js) 的耦合更加紧密。
