...
`jump 数字` 告诉时间轴在匹配sync成功时跳转至指定的时间点。 跳转至0意味着停止时间轴。 通常用于阶段跳转或循环。 尽管我们一般跳转至时间轴中已有的其他条目的时间点， 但并不意味着一定要跳转到已有的时间点。

### 指令

您可以使用 `hideall` 隐藏某个技能名。 绝大多数时间轴都会定义 `hideall "--sync--"` 来隐藏与游戏战斗阶段同步化的条目，因为玩家没有必要看到这些控制性条目。
